...
\begin{itemize}
    ...
    \item \textbf{Desenvolvimento da Aplicação de \textit{Backoffice}:} Construir uma aplicação de gestão interna para os operadores da empresa, que centralize o controlo de clientes, a triagem e agendamento de pedidos de recolha, a visualização de pesagens e a administração de utilizadores e permissões. O objetivo é substituir o uso de folhas de cálculo e comunicaç?    \item \textbf{Desenvolvimento do Terminal de Pesagem (\textit{Kiosk}):} Desenvolver uma aplicação dedicada à zona física de pesagens, concebida para operar em ecrãs táteis. O terminal simplifica a introdução de valores de peso, automatizando a recolha de dados de pesagem e associando-as à recolha em curso e ao material pesado.
    ...
    \item \textbf{Garantia de Qualidade Através de Testes Automáticos:} Assegurar a estabilidade do sistema mediante a adoção de uma metodologia de desenvolvimento guiado por testes (\textit{Test-Driven Development} --- TDD), com a implementação de uma suite de testes.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Estagiário (autor deste relatório):} Responsável principal pelo desenvolvimento do website institucional (\textit{Next.js} 15 e \textit{Tailwind CSS} v4), incluindo a área pública, a área reservada de clientes e a integração com a \ac{API} central. Participou igualmente na modelação da base de dados e na definição dos \textit{endpoints} da \ac{API} associados ao website.

    \item \textbf{Colega de estágio:} Responsável principal pelo desenvolvimento da aplicação de \textit{backoffice} e do terminal de pesagem \textit{Kiosk} (\textit{Blazor Server}, .NET 10), incluindo o sistema de emparelhamento de dispositivos.
\end{itemize}

Apesar desta divisão, ambos os estagiários colaboraram em decisões conjuntas de arquitetura, na definição de contratos da \ac{API} e na garantia de consistência de dados entre as plataformas.

\subsection{Fase 4: Testes e Validação}

Em paralelo com o desenvolvimento, foram elaborados testes automáticos que acompanharam o crescimento do projeto. Adotou-se uma abordagem de desenvolvimento guiado por testes (\textit{Test-Driven Development}), em que os testes são escritos antes ou durante a implementação de cada funcionalidade.

A suite de testes abrangeu três níveis:
\begin{itemize}
    \item \textbf{Testes Unitários:} validação isolada das regras de negócio implementadas na camada de domínio;
    \item \textbf{Testes de Integração:} validação do comportamento dos serviços da \ac{API} em conjunto com a camada de infraestrutura;
    \item \textbf{Testes de Componente (\textit{bUnit}):} validação do comportamento e renderização dos componentes \textit{Blazor} da interface de utilizador.
\end{itemize}
