...
A aplicação foi construída com a biblioteca React e a ferramenta de compilação Vite para a interface web. O processamento dos ficheiros locais é assegurado pela biblioteca XLSX, enquanto o armazenamento do histórico de tendências de preços é gerido no navegador através de IndexedDB. Implementou-se uma camada de empacotamento nativo com o Tauri para possibilitar a sua distribuição como aplicação de ambiente de trabalho (\textit{desktop}): ao contrário de soluções baseadas em Electron, o Tauri gera executáveis significativamente mais leves, o que é vantajoso em computadores mais antigos, e permite que a ferramenta funcione totalmente offline sem necessidade de ligação à internet ou dependências externas. Os testes unitários do motor de normalização de dados foram implementados com a biblioteca Vitest.
