...
O website institucional foi desenvolvido utilizando a versão 15 do Next.js com a arquitetura App Router. Esta estrutura possibilita a renderização do lado do servidor (\textit{Server-Side Rendering}) e a geração estática de páginas para diminuir o tempo de resposta inicial, garantindo simultaneamente a indexação em motores de busca (SEO). A estilização responsiva da interface foi concebida com a versão 4 do Tailwind CSS, o que resulta numa interface fluida e adaptável a smartphones, tablets e computadores.
...
Durante as fases de desenvolvimento e testes de validação com a equipa da empresa, o website institucional foi alojado na plataforma Vercel. O fluxo de desenvolvimento foi estruturado com recurso ao editor de código \textit{VS Code} e controlo de versões com \textit{Git}. Ao efetuar o envio de novos commits para o repositório alojado no \textit{GitHub}, a plataforma Vercel executava automaticamente o processo de compilação e publicação contínua (\textit{Continuous Deployment}). Note-se que o alojamento no Vercel serviu como um ambiente de testes dinâmico, facilitando a validação em tempo real das funcionalidades e da usabilidade responsiva da interface em múltiplos dispositivos físicos, incluindo computadores de secretária (\textit{desktop}) e smartphones (\textit{Android}), antes da transição para os servidores corporativos permanentes da empresa.
...
A aplicação foi construída com a biblioteca React e a ferramenta de compilação Vite para a interface web. O processamento dos ficheiros locais é assegurado pela biblioteca XLSX, enquanto o armazenamento do histórico de tendências de preços é gerido no navegador através de IndexedDB. Implementou-se uma camada de empacotamento nativo com o Tauri para possibilitar a sua distribuição como aplicação de ambiente de trabalho (\textit{desktop}). O desenvolvimento desta ferramenta foi realizado em parceria com o colega de estágio, tendo os testes unitários do motor de normalização de dados sido implementados com a biblioteca Vitest.
...
A aplicação partilha componentes reutilizáveis através do módulo comum do projeto e foi desenhada com foco na usabilidade e fiabilidade das operações sob condições de armazém. O desenvolvimento técnico específico deste módulo de quiosque ficou a cargo do colega de estágio, tirando partido da arquitetura de base e das diretrizes de design planeadas em conjunto.
